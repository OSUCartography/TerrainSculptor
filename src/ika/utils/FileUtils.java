...
public class FileUtils {
     ...
     * @return True if the file ends with the required extension, false otherwise.
     */
    ...
    /**
     * If the file at filePath exists, a warning message is displayed to the user
     * and true is returned. This method is intended to be used after a dialog
     * for specifying the name and path of a new file. If an extension is
     * required, but the user does not enter an extension, the extension can
     * be added programmatically. If this new concatenated file path conflicts
     * with an existing file, a warning should be displayed and the operation
     * should be aborted.
    ...
    /**
     * Change the extension of a file path. The extension is what follows
     * the last dot '.' in the path. If no dot exists in the path, the passed
     ...
     * @return A new path to a file. The file may not actually exist on the
     * hard disk.
     */
    ...
    /**
     * Ask the user for a file to load or to write to.
     * Makes sure the dialog is displayed in the event dispatch thread.
     * @param frame A Frame for which to display the dialog. Cannot be null.
     * @param message A message that will be displayed in the dialog.
     * @param load Pass true if an existing file for reading should be selected.
    ...
    /**
     * Converts the contents of a file into a CharSequence suitable for use
     * by the regex package.
     * The matching routines in java.util.regex require that the input be a
     * CharSequence object. This method efficiently returns the contents of a
     * file in a CharSequence object.
     * Based on http://javaalmanac.com/egs/java.util.regex/FromFile.html?l=rel
